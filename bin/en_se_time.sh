cat << EOF > /usr/share/i18n/locales/en_SE
escape_char /
comment_char %
...
localedef -i en_SE -f UTF-8 en_SE.UTF-8
grep -qE "^en_SE" /etc/locale.gen || echo "en_SE.UTF-8 UTF-8" >> /etc/locale.gen
sed -iE "s/^LC_TIME.*/LC_TIME=en_SE.UTF-8/" /etc/locale.conf
locale-gen

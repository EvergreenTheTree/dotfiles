#!/usr/bin/python2
"""
A script to unball vimball archives.  Original source:
http://zurb.com/forrst/posts/An_unball_script_for_vimball_plugins-CHM
"""
...
if len(sys.argv) < 2:
    raise SystemExit('Please specify a vba file (and optionally the '
                     'directory to unball it to)')
...
if os.path.exists(vba_dir):
    raise SystemExit('The location ' + vba_dir + ' already exists.'
                     'Please delete/move it or give a different folder'
                     'to extract into.')
